...
% Signal logging stays OFF: it streams to the SDI .dmr repository on the BOOT disk,
% which reached 8.4 GB in 25 min of the full mission and filled it. The currents now
% come from To Workspace blocks inside PMSM_Drive (pmsm_Iabc/Idq/Idc/Vdc/SOC), which
% keep the data in memory and land in the .mat on the SSD. Do NOT call
% Simulink.sdi.setRecordData(false), it disables every output including To Workspace.
set_param(cfgTop,'SignalLogging','off');
% Stop SDI mirroring the logged workspace data. Without this its .dmr repository on
% the BOOT disk grows about 264 MB per minute of wall time even with signal logging
% off, because it records the To Workspace signals too. Do NOT use
% Simulink.sdi.setRecordData(false) instead, that kills every output.
for zzP = {'InspectSignalLogs','SimulationDataInspector','RecordInternalData'}
    try, set_param(cfgTop, zzP{1}, 'off'); fprintf('  %s = off\n', zzP{1}); catch, end
end

% 50 us fixed step. The averaged inverter has no switching, so the fastest dynamics
% are the 1 kHz current loop (20 points per cycle here) and the electrical frequency,
% 316 Hz at the peak rotor speed (63 points per cycle). 2.5x faster than 20 us.
% VTOLDynamics is a referenced model and carries its own FixedStep of 2e-5. That
% 20 us becomes a sample time in the top model, so the top step has to divide it.
% Both config sets must be set, or the top one alone is rejected.
zzStep = '50e-6';
set_param(cfgTop,'FixedStep',zzStep);
zzSub = cfgSubActive;
try, zzSub = getRefConfigSet(cfgSubActive); catch, end
try
    set_param(zzSub,'SolverType','Fixed-step','Solver','ode4','FixedStep',zzStep);
catch zzE
    fprintf(2,'sub config: %s\n', zzE.message);
end
fprintf('fixed step: top=%s  sub=%s\n', get_param(cfgTop,'FixedStep'), get_param(zzSub,'FixedStep'));

try, Simulink.sdi.clear; catch, end                 % free C-drive SDI archive
zzT = getenv('SIM_STOPTIME'); if isempty(zzT), zzT = '60'; end
set_param(cfgTop,'StopTime',zzT);
fprintf('\n>>> running %s s sim (Jm_flight=%.4g), started %s\n', zzT, Jm_flight, datestr(now,'HH:MM:SS'));
...
if strcmp(zzAccel,'1')
    try
        ...
        fprintf(2,'accelerator not available: %s\n', zzE.message);
    end
else
    set_param(mdl,'SimulationMode','normal');
    fprintf('simulation mode = normal\n');
end
...
try
    ...
    save(['/Volumes/PortableSSD/files/maltab/kiat_pmsm/pmsm_run_' zzT 's.mat'],'V','-v7.3');
    fprintf('saved pmsm_run_%ss.mat\n', zzT);
catch e
...
save(['/Volumes/PortableSSD/files/maltab/kiat_pmsm/run_out_' zzT 's.mat'],'R','-v7.3');
