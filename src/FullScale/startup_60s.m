...
%% Run Simulink
%% --- controlled test: same setup as a normal startup run, stopped at 60 s ---
zzT = getenv('SIM_STOPTIME'); if isempty(zzT), zzT = '60'; end
set_param(getRefConfigSet(getActiveConfigSet(mdl)),'StopTime',zzT);
fprintf('>>> startup.m setup, StopTime=%s s\n', zzT);
outTuned = sim(mdl);
zzXe = squeeze(outTuned.UAV_State.Xe.Data); if size(zzXe,1)==3, zzXe=zzXe.'; end
S.alt=-zzXe(:,3); S.t_alt=outTuned.UAV_State.Xe.Time;
S.rpm=[squeeze(outTuned.Rotor1_RPM.Data(:)) squeeze(outTuned.Rotor3_RPM.Data(:))];
S.t_rpm=outTuned.Rotor1_RPM.Time;
S.ref=squeeze(outTuned.Rotor1_RPM_Reference.Data(:)); S.t_ref=outTuned.Rotor1_RPM_Reference.Time;
S.mode=double(outTuned.Flight_Mode.Data); S.t_mode=outTuned.Flight_Mode.Time;
S.tilt=squeeze(outTuned.UAV_State.RotorParameters.Tilt1.Data(:)); S.t_tilt=outTuned.UAV_State.RotorParameters.Tilt1.Time;
save(['/Volumes/PortableSSD/files/maltab/kiat_pmsm/startup_' zzT 's.mat'],'S','-v7.3');
fprintf('alt max=%.1f final=%.1f m | rotor1 final=%.0f rpm | tilt final=%.1f deg\n', ...
        max(S.alt), S.alt(end), S.rpm(end,1), S.tilt(end)*180/pi);
fprintf('saved startup_%ss.mat\n', zzT);
