...
% Battery Model Parameters (To compute soc, use battery_sys.maximum_capacity)
...
HEV_Param.Battery_Cell.Rated_Capacity     = 5;              % Ampere-hours (배터리 정격 용량)
HEV_Param.Battery_Cell.SOC_init           = 0.9;            % 배터리 초기 SOC 값
...
HEV_Param.Battery_Cell.Area               = 0.01;           % (m^2) Surface area of battery exposed to air (열 전달이 일어나는 면적)
HEV_Param.Battery_Cell.Rtheta             = 20;             % (W/m^2/K) Convective heat transfer coefficient (대류 계수)
...
HEV_Param.Battery_Cell.Emo                = 4.8*4;     % (V) [max o.c. volts per cell] (open circuit voltage at full charge)
HEV_Param.Battery_Cell.R0                 = 0.001;     % (Ohm)
HEV_Param.Battery_Cell.R1                 = 0.01;      % 배터리 셀 저항 값 (Ohm)
HEV_Param.Battery_Cell.R2                 = 0.05;      % 배터리 셀 저항 값 (Ohm)
...
HEV_Param.Motor.TorqSpdLUT.TorqueNm          = [40 40 40 40 40 40 35 30 20 15 0.01]*1;
...
HEV_Param.Motor.Damping                      = 1e-3;        % N*m/(rad/s)
...
HEV_Param.Motor.Efficiency                   = 91;          % 모터 효율

%% [4] : DC-DC CONVERTER PARAMETERS
